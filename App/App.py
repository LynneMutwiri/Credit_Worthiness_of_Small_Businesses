...
def main():
    ...
    NoEmp = int(st.number_input("Number of employees?",value=0, step=1))
    ...
    if MembershipStatus == 'No':
        result = 'You need to be an SBA member to apply. Kindly get in touch or visit our [website](https://www.sba.gov/) to apply '
    else: 
        if PrevLoanStatus == 'No':
            result= 'Sorry. The SBA does not guarantee new loans to businesses who have not repaid their previous loan. \
                Kindly repay your previous loan in order to apply for new loan'
        else:
            if prediction[0] == 0:
                result= '''Sorry, your loan request was not successful. \n \
                \n Please check on the following to improving your chances of getting an SBA loan: \n \
                \n 1. Find a suitable balance between loan amount and repayment period \n \
